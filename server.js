...
app.use(
  cors({
    origin: ["https://mem-project.vercel.app", "http://localhost:3000"],
  }),
);
...
app.get("/", (req, res) => {
  res.sendFile(
    path.join(__dirname, "../frontend", "HTMLPAGES/next_tut/app/page.js"),
  );
});
...
app.get("/login", async (req, res) => {
  ...
  const token = jwt.sign({ userId: user.id, userRole: user.ROLE }, jwt_key, {
    expiresIn: "7d",
  });
  ...
  try {
    ...
    const contactsWithMessages = await Promise.all(
      contacts.map(async (contactId) => {
        ...
        const hashedContactId = jwt.sign({ contactId: contactId }, jwt_key, {
          expiresIn: "1d",
        });
...
app.get("/getpic", (req, res) => {
  const imgPath = path.join(
    __dirname,
    "../frontend",
    "/imgs/annie-spratt-QckxruozjRg-unsplash.jpg",
  );
